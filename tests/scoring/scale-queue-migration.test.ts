...
describe("score recalculation queue scaling migration", () => {
  it("prioritizes current evidence with independently bounded branches", async () => {
    ...
    assert.match(sql, /version_mismatch as \(/);
    assert.match(sql, /current_signals as materialized \(/);
    assert.match(sql, /actionable_missing as \(/);
  ...
  it("keeps the exact recalculation triggers and stays read-only", async () => {
    ...
    assert.match(sql, /h\.status in \('online', 'degraded', 'offline'\)/);
    assert.match(sql, /r\.source is not null/);
