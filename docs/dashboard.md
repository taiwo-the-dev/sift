...
1. The server creates a random five-minute challenge bound to the request origin, wallet, and BSC Testnet.
...
The timeline is built only from `job_activity`. Every entry is labelled as an application record, indexed observation, or on-chain evidence. Transaction links are emitted only for valid BSC Testnet hashes. No activity record means unavailable activity evidence, not proven inactivity.
...
4. Open `/dashboard` disconnected, on the wrong network, and with a BSC Testnet wallet.
