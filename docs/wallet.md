...
Sift configures these typed viem/wagmi chains in this order:

1. BSC Testnet — chain ID `97` — default development and initial chain.
2. BNB Smart Chain — chain ID `56` — available as an explicit network choice.

An unsupported connected network is shown honestly and offers an explicit switch to BSC Testnet. Sift never switches a wallet automatically; the provider must ask the user for consent. If programmatic switching is unavailable, the interface instructs the user to select chain ID `97` manually.
...
- connected address shortened to `0x1234…5678` and the actual provider network;
- unsupported networks with an explicit BSC Testnet switch;
...
5. Select an unsupported network in the wallet, return to Sift, and choose **Switch to BSC Testnet**.
6. Approve the switch and confirm chain ID `97`. Repeat and reject the switch to verify cancellation guidance.
