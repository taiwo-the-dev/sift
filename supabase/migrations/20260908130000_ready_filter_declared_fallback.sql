-- M22 follow-up: the "Available" discovery filter and the homepage carousel
-- both call search_ready_agents. The original readiness gate required a service
-- with a fresh successful activation probe (availability_status = 'available'
-- within 24 hours). Until the bounded activation queue has probed the hosted
-- catalogue, every service is still 'unchecked', so the filter returned nothing.
--
-- This relaxes the gate to also accept a service that declares a supported
-- activation method with an endpoint and has not yet been probed
-- ('unchecked'). A service that was probed and came back 'degraded',
-- 'unavailable', or 'unsupported' is still excluded, and a stale 'available'
-- observation (older than 24 hours) is still excluded. The per-agent card and
-- profile continue to derive their "Available" badge from the observed service
-- evidence alone, so an unproven declaration is never shown as a confirmed
-- execution claim.
    ...
    where a.chain_id = any(v_chain_ids)
      ...
      and exists (
        ...
        where available_service.agent_db_id = a.id
          ...
          and (
            (
              available_service.availability_status = 'available'
              and available_service.availability_last_success_at
...
comment on function public.search_ready_agents(
  ...
  'Bounded discovery search for agents with a usable task service: a supported activation method that either passed a live check in the last 24 hours or has not yet been probed. Degraded, unavailable, unsupported, and stale services are excluded.';
