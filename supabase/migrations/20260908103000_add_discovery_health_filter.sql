...
create index if not exists agent_health_status_agent_idx
  on public.agent_health (status, agent_db_id);

comment on index public.agent_health_status_agent_idx is
  'Supports discovery filtering by the latest persisted agent health status.';

create function public.search_agents_with_health(
...
begin
  select coalesce(array(
    ...
    from unnest(coalesce(p_chain_ids, array[]::bigint[]))
      as supplied(chain_id)
  ...
  select coalesce(array(
    ...
    where supplied.health_status in (
      'online', 'degraded', 'offline', 'unknown'
    )
    order by supplied.health_status
    limit 4
  ), array[]::text[])
  into v_health_statuses;
  ...
  if cardinality(v_health_statuses) = 0 then
    raise exception 'At least one supported health status is required.';
  end if;

  if 'unknown' = any(v_health_statuses) then
    v_from_sql := v_from_sql ||
      ' left join public.agent_health as matched_health ' ||
      'on matched_health.agent_db_id = a.id';
    v_where_sql := v_where_sql ||
      ' and coalesce(matched_health.status, ''unknown'') = any($7)';
  else
    v_from_sql := v_from_sql ||
      ' inner join public.agent_health as matched_health ' ||
      'on matched_health.agent_db_id = a.id ' ||
      'and matched_health.status = any($7)';
  end if;
...
revoke execute on function public.search_agents_with_health(
...
comment on function public.search_agents_with_health(
  text[], text[], text[], text, integer, integer, bigint[], text[]
) is
  'Bounded agent discovery query with an evidence-based health status filter. Missing health observations are represented by the unknown filter.';
