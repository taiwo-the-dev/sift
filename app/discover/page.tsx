...
import {
  DiscoveryNavigationProvider,
  DiscoveryResultsFrame,
} from "@/components/discovery/discovery-navigation";
...
export default async function DiscoverPage({ searchParams }: DiscoverPageProps) {
  ...
  const resultsKey = buildDiscoveryHref(query);
  const routeStateKey = JSON.stringify([selectedNetwork, params]);

  return (
    <DiscoveryNavigationProvider routeKey={routeStateKey}>
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,0.78fr)_minmax(28rem,1.22fr)] lg:items-end">
              <div>
                ...
                <p className="mt-5 max-w-xl text-pretty text-base leading-7 text-muted-foreground">
                  Search BNB Chain agents by service, capability, and latest
                  health check.
                </p>
              </div>
